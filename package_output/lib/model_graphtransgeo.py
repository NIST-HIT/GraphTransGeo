...
class GraphTransGeo(TrustGeoSpatial):
    """
    ...
    def __init__(self, dim_in, epsilon=0.01, alpha=0.01, beta=0.5):
        super(GraphTransGeo, self).__init__(dim_in)
    ...
    def forward(self, lm_X, lm_Y, tg_X, tg_Y, lm_delay, tg_delay, add_noise=0, training=True):
        ...
        """
        # 标准视角前向传播
        clean_outputs = super(GraphTransGeo, self).forward(lm_X, lm_Y, tg_X, tg_Y, lm_delay, tg_delay, add_noise)
        ...
        if training:
            ...
            # 前向传播计算损失
            temp_outputs = super(GraphTransGeo, self).forward(
            ...
            # 对抗视角前向传播
            adv_outputs = super(GraphTransGeo, self).forward(
