...
class SimpleGCN(nn.Module):
    ...
    def adversarial_loss(self, x, edge_index=None, edge_attr=None, batch=None, alpha=0.01, beta=0.5, epsilon=None):
        ...
        # Apply beta weight here so it's not applied again in training loop
        weighted_loss = beta * consistency_loss
        
        return pred_clean, weighted_loss

class HaversineLoss(nn.Module):
    def __init__(self, radius=6371.0, scale_factor=1.0):
        super(HaversineLoss, self).__init__()
        self.radius = radius  # Earth radius in kilometers
        self.scale_factor = scale_factor  # Scale factor for normalized coordinates
    
    def forward(self, pred, target):
        # Handle normalized coordinates (0-1 range)
        # For normalized coordinates, we need to scale them back to degrees
        # The scaling is handled by denormalization in the test function
        
        # Convert to radians (assuming coordinates are in degrees)
        # For normalized coordinates (0-1), the conversion is less extreme
        pred_lat = pred[:, 0] * math.pi / 180
        pred_lon = pred[:, 1] * math.pi / 180
        target_lat = target[:, 0] * math.pi / 180
        target_lon = target[:, 1] * math.pi / 180
        
        # Calculate Haversine distance with improved numerical stability
        ...
        a = torch.clamp(a, min=0.0, max=1.0)  # Ensure a is in [0, 1]
        
        c = 2 * torch.asin(torch.sqrt(a))
        
        # Apply radius and scale factor
        # For normalized coordinates, the scale factor can be adjusted
...
class CombinedLoss(nn.Module):
    def __init__(self, hav_weight=0.3, adaptive=True, alpha=0.5):
        super(CombinedLoss, self).__init__()
        self.mse_loss = nn.MSELoss()
        self.haversine_loss = HaversineLoss()
        self.hav_weight = hav_weight
        self.adaptive = adaptive
        self.alpha = alpha  # Controls adaptation rate
        self.running_mse = 1.0
        self.running_hav = 1.0
    ...
    def forward(self, pred, target):
        ...
        if self.adaptive:
            # Update running averages for loss components
            self.running_mse = self.alpha * self.running_mse + (1 - self.alpha) * mse.item()
            self.running_hav = self.alpha * self.running_hav + (1 - self.alpha) * hav.item()
            
            # Calculate adaptive weight to balance the components
            # When Haversine is much larger than MSE, reduce its weight
            ratio = self.running_hav / (self.running_mse + 1e-8)
            adaptive_weight = self.hav_weight / (1.0 + torch.log1p(torch.tensor(ratio)).item())
            
            # Ensure weight stays in reasonable range
            adaptive_weight = max(0.05, min(0.5, adaptive_weight))
            
            # Calculate combined loss with adaptive weight
            loss = (1 - adaptive_weight) * mse + adaptive_weight * hav
        else:
            # Use fixed weight
...
def train(model, train_loader, val_loader, optimizer, criterion, device, args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for data in pbar:
            ...
            # Add adversarial loss (without applying beta again)
            loss = loss + adv_loss
...
def test(model, test_loader, criterion, device):
    ...
    norm_mse = np.mean((preds - targets) ** 2)
    norm_mae = np.mean(np.abs(preds - targets))
    
    # Get normalization parameters from the dataset
    y_min = test_loader.dataset.y_min
    y_max = test_loader.dataset.y_max
    
    # Denormalize predictions and targets for real-world evaluation
    preds_denorm = preds * (y_max - y_min) + y_min
    targets_denorm = targets * (y_max - y_min) + y_min
    
    # Calculate MSE and MAE on denormalized data
    test_mse = np.mean((preds_denorm - targets_denorm) ** 2)
    test_mae = np.mean(np.abs(preds_denorm - targets_denorm))
    
    # Calculate distance errors using denormalized coordinates
    distance_errors = []
    
    for i in range(len(preds_denorm)):
        # Calculate Haversine distance
        pred_lat = preds_denorm[i, 0] * math.pi / 180
        pred_lon = preds_denorm[i, 1] * math.pi / 180
        target_lat = targets_denorm[i, 0] * math.pi / 180
        target_lon = targets_denorm[i, 1] * math.pi / 180
    ...
    # Log normalized and denormalized metrics
    logging.info(f'Normalized - MSE: {norm_mse:.6f}, MAE: {norm_mae:.6f}')
    logging.info(f'Denormalized - MSE: {test_mse:.6f}, MAE: {test_mae:.6f}')
