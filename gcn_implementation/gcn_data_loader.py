...
class GraphDataset(InMemoryDataset):
    ...
    def __init__(self, features, labels, transform=None, pre_transform=None, k=10):
        ...
        # Initialize normalization parameters (will be set by load_dataset)
        self.y_min = None
        self.y_max = None
...
def load_dataset(dataset_name, data_dir, train_ratio=0.7, val_ratio=0.15, test_ratio=0.15, seed=42):
    ...
    # Normalize features (already done by default)
    
    # Normalize labels (coordinates) to reduce Haversine loss magnitude
    # Store original ranges for denormalization during evaluation
    y_min = np.min(train_labels, axis=0)
    y_max = np.max(train_labels, axis=0)
    
    # Apply normalization to all sets using the same scaling factors
    train_labels_normalized = (train_labels - y_min) / (y_max - y_min + 1e-12)
    val_labels_normalized = (val_labels - y_min) / (y_max - y_min + 1e-12)
    test_labels_normalized = (test_labels - y_min) / (y_max - y_min + 1e-12)
    
    logging.info(f'Labels normalized from range {y_min} - {y_max} to 0-1')
    
    # Create datasets with normalized labels
    train_dataset = GraphDataset(train_features, train_labels_normalized)
    val_dataset = GraphDataset(val_features, val_labels_normalized)
    test_dataset = GraphDataset(test_features, test_labels_normalized)
    
    # Store normalization parameters in datasets for later use
    train_dataset.y_min = y_min
    train_dataset.y_max = y_max
    val_dataset.y_min = y_min
    val_dataset.y_max = y_max
    test_dataset.y_min = y_min
    test_dataset.y_max = y_max
