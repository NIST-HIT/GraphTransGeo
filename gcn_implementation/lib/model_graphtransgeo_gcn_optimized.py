...
class OptimizedGraphTransGeoGCN(nn.Module):
    ...
    def forward(self, x, edge_index, batch=None):
        ...
        """
        # Feature scaling - normalize input features to improve stability
        # Calculate feature statistics across batch dimension
        x_mean = x.mean(dim=0, keepdim=True)
        x_std = x.std(dim=0, keepdim=True) + 1e-6  # Add small epsilon to prevent division by zero
        
        # Apply feature scaling
        x = (x - x_mean) / x_std
        
        # Input normalization with batch norm
        x = self.input_bn(x)
        ...
        for i in range(1, self.num_layers):
            ...
            # Residual connection with scaling factor to prevent feature explosion
            scale_factor = 0.8  # Slightly downweight residual to improve stability
            x_prev = x_new + scale_factor * x_prev
