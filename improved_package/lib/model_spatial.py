...
class GraphSAGE(nn.Module):
    def __init__(self, in_features, out_features, hidden_dim=42):
    ...
    def message_passing(self, adj, x):
        # 简化的消息传递机制
        # 对邻居节点特征进行聚合
        neighbor_feat = torch.matmul(adj, x)
        return neighbor_feat
    
    def forward(self, adj, x):
        # 对邻居节点特征进行聚合
        neighbor_feat = self.message_passing(adj, x)
        
        # 组合自身特征和邻居特征
        combined = neighbor_feat + x
...
class TrustGeoSpatial(nn.Module):
    def __init__(self, dim_in):
        super(TrustGeoSpatial, self).__init__()
        self.dim_in = dim_in
        self.dim_z = dim_in + 2
        ...
        # transform in Graph
        self.w_1 = GraphSAGE(self.dim_in + 2, self.dim_in + 2)
        self.w_1_l = nn.Linear(self.dim_in + 2, self.dim_in + 2)

        self.w_2 = GraphSAGE(self.dim_in + 2, self.dim_in + 2)
    ...
    def forward(self, lm_X, lm_Y, tg_X, tg_Y, lm_delay, tg_delay, add_noise=0):
        ...
        tg_feature_0 = torch.cat((tg_X, torch.zeros(N2, 2)), dim=1)
        ...
        # 确保delay_score是一维张量
        if delay_score.dim() > 1:
            delay_score = delay_score.squeeze()
        
        rou2tar_score_0 = torch.exp(-self.gamma_2 * (self.alpha * tg_delay + self.beta))
        
        # 确保rou2tar_score_0是一维张量
        if rou2tar_score_0.dim() > 1:
            rou2tar_score_0 = rou2tar_score_0.squeeze()
        
        # 确保形状匹配
        if isinstance(rou2tar_score_0, torch.Tensor) and hasattr(rou2tar_score_0, 'shape') and len(rou2tar_score_0.shape) > 0 and rou2tar_score_0.shape[0] != N2:
            rou2tar_score_0 = rou2tar_score_0.reshape(N2)
        ...
        # print(self.w_1_l(degree_mul_adj_0@all_feature_0).shape, step_1_all_feature.shape)
