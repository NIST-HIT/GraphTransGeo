#!/usr/bin/env python
# -*- coding: utf-8 -*-
# Visualize MSE and MAE metrics from training logs
...
def plot_metrics(log_file, output_dir="~/temp_images"):
    ...
    plt.plot(epochs, train_mse, 'b-', label='训练MSE')
    plt.plot(epochs, val_mse, 'r-', label='验证MSE')
    plt.title(f'{dataset}数据集MSE曲线')
    ...
    # Save figure
    mse_output = f"{output_dir}/{dataset}_mse_curve.png"
    ...
    plt.figure(figsize=(12, 8))
    plt.plot(epochs, train_mae, 'b-', label='训练MAE')
    plt.plot(epochs, val_mae, 'r-', label='验证MAE')
    plt.title(f'{dataset}数据集MAE曲线')
    plt.xlabel('训练轮次 (Epoch)')
    ...
    # Save figure
    mae_output = f"{output_dir}/{dataset}_mae_curve.png"
    ...
    # Plot MSE
    ax1.plot(epochs, train_mse, 'b-', label='训练MSE')
    ax1.plot(epochs, val_mse, 'r-', label='验证MSE')
    ax1.set_title(f'{dataset}数据集MSE曲线')
    ...
    # Plot MAE
    ax2.plot(epochs, train_mae, 'b-', label='训练MAE')
    ax2.plot(epochs, val_mae, 'r-', label='验证MAE')
    ...
    combined_output = f"{output_dir}/{dataset}_mse_mae_combined.png"
